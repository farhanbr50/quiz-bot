...
ADMINS = [560588833]  # Apni ID daalo
...
# ============================================================
# NEW COMMANDS - quiz@on | quiz@off | speed@15
# ============================================================

@dp.message(lambda message: message.text and message.text.lower().strip() == 'quiz@on')
async def start_quiz_at(message: types.Message):
    await start_quiz_handler(message)

@dp.message(lambda message: message.text and message.text.lower().strip() == 'quiz@off')
async def stop_quiz_at(message: types.Message):
    await stop_quiz_handler(message)

@dp.message(lambda message: message.text and message.text.lower().startswith('speed@'))
async def speed_command_at(message: types.Message):
    await speed_handler(message)

# ============================================================
# OLD COMMANDS - Backward Compatibility
# ============================================================

@dp.message(lambda message: message.text and message.text.lower() in ['/quiz/on', '/quiz on', '.quiz.on'])
async def start_quiz_old(message: types.Message):
    await start_quiz_handler(message)

@dp.message(lambda message: message.text and message.text.lower() in ['/quiz/off', '/quiz off', '.quiz.off'])
async def stop_quiz_old(message: types.Message):
    await stop_quiz_handler(message)

@dp.message(lambda message: message.text and (message.text.lower().startswith('/sp') or message.text.lower().startswith('.speed')))
async def speed_old(message: types.Message):
    await speed_handler(message)
...
async def speed_handler(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer("❌ Usage: speed@15 or /sp 15 or .speed 15")
        return
    ...
    if speed_input.endswith('m'):
        ...
        try:
            ...
            if speed_seconds < 3:
                await message.answer("❌ Minimum 3 seconds!")
                return
        except:
...
async def main():
    ...
    print("🚀 MAHAKAL QUIZ Bot starting...")
    print("📌 Commands: quiz@on | quiz@off | speed@15")
