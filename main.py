...
# ============================================================
# NEW COMMANDS - .quiz.on .quiz.off .speed
# ============================================================
...
async def send_question_results(chat_id, question_idx):
    ...
    if not answers:
        await bot.send_message(chat_id, f"📊 Q{question_idx+1}: ❌ No one answered!")
        return
    ...
    result = f"📊 *Q{question_idx+1} Results*\n\n"
    result += f"📌 {question_text}\n"
    result += f"✓ Correct: {options[correct_option]}\n\n"
